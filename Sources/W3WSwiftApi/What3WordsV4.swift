...
public class What3WordsV4: W3WRequest, W3WProtocolV4 {
  ...
  public func copy() -> What3WordsV4 {
    return What3WordsV4(apiKey: apiKey, apiUrl: baseUrl, headers: headers)
  }
  ...
  /**
   ...
   - parameter completion: A W3WGridResponse completion handler
   */
  ...
  /**
   Returns a section of the 3m x 3m what3words grid for a given area.
   - parameter bounds: A W3WBox describing the area to use
   - parameter completion: A W3WGridResponse completion handler
   */
  public func gridSection(bounds: W3WSwiftCore.W3WBox, completion: @escaping W3WSwiftCore.W3WGridResponse) {
    self.gridSection(south_lat: bounds.southWest.latitude, west_lng: bounds.southWest.longitude, north_lat: bounds.northEast.latitude, east_lng: bounds.northEast.longitude, completion: completion)
  }
  ...
  func toParameters(from: [W3WOption]?, with: [String: String] = [:]) -> [String: String] {
    ...
    for option in from ?? [] {
      print(option.key().description, option.forQueryString())
      params[option.key().description] = option.forQueryString()
    }
