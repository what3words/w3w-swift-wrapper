...
//options: BoundingBox(southWest: , northEast:
  ...
  func testAutosuggestNumberResults() {
    ...
    func testVoiceAutosuggest() {
        ...
        W3wGeocoder.shared.autosuggest(input: json.removingPercentEncoding ?? "", options: [InputType(inputType: .voconHybrid), FallbackLanguage(language: "en"), NumberResults(numberOfResults: 3), Focus(focus: CLLocationCoordinate2D(latitude: 51.4243877, longitude: -0.3474524))]) { (suggestions, error) in
    ...
    func testGenericVoiceAutosuggest() {
        ...
        W3wGeocoder.shared.autosuggest(input: "filled count soap", options: InputType(inputType: .genericVoice), FallbackLanguage(language: "en"), NumberResults(numberOfResults: 3)) { (suggestions, error) in
